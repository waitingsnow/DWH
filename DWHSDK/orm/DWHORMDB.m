		 ...
		 SQLITE_OK)) {
			...
			@try{
				...
				for (int y = 0; y < columnCount; y++) {
					...
					if ([columnName compare:column] == NSOrderedSame) {
						return  YES;
					}
				}
			}@finally{
				...
				for (DWHORMDBClassPropertyInfo *info in obj.propertyInfos) {
					if (info.type != ORMDBDataTypeClass &&
						...
						if (foreignSignature) {
							...
							NSMutableArray *a =
							[DWHORMDB queryDB:info.protocol ? NSClassFromString(info.protocol)
										  : info.cls
									andSql:sql];
							id obj = a;
							...
							if (info.cls && [obj isKindOfClass:[NSMutableArray class]]) {
								NSLog(@"================================== ormDB error:%@ %@ sql:%@",info.cls,a,sql);
							}
...
+(void)saveObject:(id)entity withSql:(NSString *)sql{
	...
	sqlite3_stmt *statement;
	if (sqlite3_prepare_v2(DWHdatabase, [sql UTF8String], -1, &statement, NULL) == SQLITE_OK) {
		for (DWHORMDBClassPropertyInfo *info in obj.propertyInfos) {
